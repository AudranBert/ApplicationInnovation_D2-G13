...
def full_train_with_valid(nb_epoch=2):
    ...
    for epoch in range(1, nb_epoch + 1):  # train
        model.train()  # tell that the model will be trained
        t_loss = 0
        t_batch = 0
        for batch_idx, (x, target) in enumerate(
            tqdm(train_loader, desc=str(epoch) + ": Train batch")):  # for each batch
            model.train()
            ...
            if get_step(epoch + 1, train_loader, batch_idx) % 1000 == 0 and batch_idx != 0:
                size = len(target) * t_batch
                step_loss = t_loss / size
                writer.add_scalar('Loss/train', step_loss, get_step(epoch, train_loader, batch_idx))
                t_loss = 0
                t_batch = 0
            if get_step(epoch + 1, train_loader, batch_idx) % 25000 == 0 and batch_idx != 0:
                torch.save(model, checkpoints_folder + "/last_model.pth")
                ...
                if v_acc < best_valid_acc:  # keep the best weights
                    ...
                    torch.save(model, checkpoints_folder + "/best_model.pth")
        torch.save(model, checkpoints_folder + "/last_model.pth")
...
def test():
    ...
    model = torch.load(checkpoints_folder+"/best_model.pth")  # load the best weights of the model and use to test
    time.sleep(0.01)
