...
def train_epoch(epoch, model, optimizer, criterion, train_loader, valid_loader):
    ...
    for batch_idx, (x, target) in enumerate(tqdm(train_loader, desc=str(epoch) + ": Train batch")):  # for each batch
        ...
        if batch_idx % 500 == 100:
            ...
            t_loss = 0
            t_batch = 0
        if batch_idx % 25000 == 24999:
            valid_epoch(str(epoch)+str(batch_idx), model, valid_loader)


def valid_epoch(epoch: str, model, valid_loader):
    model.eval()
    correct_tot = 0
    with torch.no_grad():
...
def train():
    ...
    for epoch in range(nb_epoch):  # train
        train_epoch(epoch + 1, model, optimizer, nn.CrossEntropyLoss(), train_loader, valid_loader)
        v_acc = valid_epoch(str(epoch + 1), model, valid_loader)  # test validation
